...
#pragma resource "*.dfm"
TFormReportBUCityDay *FormReportBUCityDay;

extern EDRegOpts opts;

//---------------------------------------------------------------------------
AnsiString __fastcall TFormReportBUCityDay::GetPeriodStr(TDateTime dt)
{
  AnsiString Str="";
...
//---------------------------------------------------------------------------
__fastcall TFormReportBUCityDay::TFormReportBUCityDay(TComponent* Owner)
        : TForm(Owner)
{
}
//---------------------------------------------------------------------------
void __fastcall TFormReportBUCityDay::InitReportQuery(void)
{
...
//---------------------------------------------------------------------------
AnsiString __fastcall TFormReportBUCityDay::GetSummCity(AnsiString CityIDStr,AnsiString NumYearStr,AnsiString NumMounthStr,AnsiString NumDayStr)
{
  int Summ=0;
...
//  MessageBox(Handle,PatternStr.c_str(),"",MB_OK);
  ...
  ZMySqlQuerySumm->SQL->Add("select pf.moneypay,pf.datepay,voc.title from "+opts.DBPayFacts+" as pf,"+opts.DBVocTable+" as voc,"+opts.DBStudTable+" as st where pf.deleted=0 and st.deleted=0 and voc.deleted=0 and voc.vkey=\'grp\' and (voc.title like "+ToStr(Pattern1Str)+" or voc.title like "+ToStr(Pattern2Str)+") and st.grpid=voc.num and pf.idstud=st.id and YEAR(pf.datepay)="+ToStr(NumYearStr)+" and MONTH(pf.datepay)="+ToStr(NumMounthStr)+" and DAYOFMONTH(pf.datepay)="+ToStr(NumDayStr)+" order by pf.datepay");
//MessageBox(Handle,ZMySqlQuerySumm->Sql->Strings[0].c_str(),"",MB_OK);
...
//---------------------------------------------------------------------------
void __fastcall TFormReportBUCityDay::ToolBtnCloseClick(
      TObject *Sender)
{
  Close();
}
//---------------------------------------------------------------------------
void __fastcall TFormReportBUCityDay::FormClose(TObject *Sender,
      TCloseAction &Action)
{
  Action=caFree;
}
//---------------------------------------------------------------------------
void __fastcall TFormReportBUCityDay::CreateWordDocument(void)
{
    InitReportQuery();
...
//  WordDocument->Tables->Item(CountTables)->Range->Select();
    ...
    macros.TablesCell(CountTables, 1, 3, "Select");
    macros.SelectionParagraphFormat("Alignment = wdAlignParagraphCenter");
    ...
    for (int i=0;i<ZMySqlQuery->RecordCount;i++)
    {
        ...
        SummStr=GetSummCity(ZMySqlQuery->Fields->FieldByNumber(2)->AsString,AnsiString(year),AnsiString(month),AnsiString(day));
...
//---------------------------------------------------------------------------
void __fastcall TFormReportBUCityDay::ToolBtnPreviewClick(
      TObject *Sender)
{
  try
  {
...
//---------------------------------------------------------------------------
void __fastcall TFormReportBUCityDay::FormCreate(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TFormReportBUCityDay::FormDestroy(TObject *Sender)
{
  pFormSelDate->Free();
  WCDisconnect();
}
