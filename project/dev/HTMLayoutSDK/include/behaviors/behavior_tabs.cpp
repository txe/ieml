...
struct tabs: public behavior
{
    ...
    bool select_tab( dom::element& tabs_el, dom::element& tab_el )
    {
      ...
      if( prev_panel_el.is_valid() )
      {
        ...
        prev_panel_el.set_state(STATE_COLLAPSED,0,false); // set collapsed in case of someone use it for styling
        prev_panel_el.update(RESET_STYLE_THIS);
      }
      if( prev_tab_el.is_valid() )
      {
        prev_tab_el.set_attribute("selected", 0); // remove selected attribute
        prev_tab_el.set_state(0,STATE_CURRENT,false); // reset also state flag, :current
        prev_tab_el.update(RESET_STYLE_DEEP); // highly propbable that it has 
      }
      
      panel_el.set_attribute("selected", L""); // set selected attribute (empty)
      panel_el.set_state(STATE_EXPANDED,0,false); // expand it
      panel_el.update(RESET_STYLE_THIS);
      ...
      tab_el.update(RESET_STYLE_DEEP);
            
      tabs_el.update(MEASURE_INPLACE | RESET_STYLE_THIS);  //done! update changes on the view
