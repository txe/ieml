...
struct actions: public behavior
{
    ...
    bool interpret_action( dom::element root, const wchar_t* action )
    {
      ...
      if( a.like(L"alert:*"))
      {
        aux::wchars msg;
        if(parse_args(a,msg))
          ::MessageBoxW(NULL,msg.start,L"alert!",MB_OK);
        return true;
      }
      else if( a.like(L"copy-value:*")) // copy-value: dst selector , src selector
      {
        ...
        if(!parse_args(a,dst_sel,src_sel))
          return true;
        dom::element src = root.find_first(aux::w2a(src_sel));
        dom::element dst = root.find_first(aux::w2a(dst_sel));
    ...
    bool action_copy_state( dom::element root, aux::wchars cmd)
    {
      ...
      struct : dom::callback 
      {
        int  state_to_set;
        int  state_to_clear;
        bool on_element(HELEMENT he) { htmlayout::dom::element el = he; el.set_state(state_to_set,state_to_clear); 
      ...
      root.find_all(&actor, aux::w2a(dst_sel));
